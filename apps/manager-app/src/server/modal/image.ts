...
export function getSessionImage(): Image {
  ...
  image = modal.images
    ...
    .dockerfileCommands([
      ...
      "RUN mkdir -p /opt/session-runtime-api && cd /opt/session-runtime-api && npm init -y && npm install hono @hono/node-server @hono/zod-validator @anthropic-ai/claude-agent-sdk",
