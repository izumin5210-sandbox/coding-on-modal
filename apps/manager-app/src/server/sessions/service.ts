...
import {
  callSessionRuntimeAgent,
  callSessionRuntimeExec,
  ensureSessionRuntimeApiReady,
} from "@/server/sessions/runtime-api";
...
export { SessionError } from "@/server/sessions/errors";
...
async function resolveHostKeyMetadata(providerSessionId: string): Promise<{
  ...
  const result = await runSessionSandboxCommand(
    providerSessionId,
    [
      "sh",
      "-lc",
...
export async function createSession(
  ...
  try {
    ...
    const cloneResult = await runSessionSandboxCommand(
