...
import { agentResponseSchema } from "session-runtime-api/contract";
import type {
  AgentSessionInput,
  CreateSessionInput,
  ExecSessionInput,
...
async function deploySessionRuntimeApi(
  ...
  const directories = new Set<string>([SESSION_RUNTIME_API_DIR, SESSION_RUNTIME_API_DIST_DIR]);
  ...
  const mkdirCommand = `mkdir -p ${[...directories]
    ...
    .join(" ")}`;
  const mkdirResult = await runCommand(providerSessionId, ["sh", "-lc", mkdirCommand]);
...
`;
  ...
  const result = await runCommand(providerSessionId, ["sh", "-lc", startScript]);
...
async function ensureSessionRuntimeApiReady(
  providerSessionId: string,
  ...
  const logResult = await runCommand(providerSessionId, [
    "sh",
    "-lc",
...
async function callSessionRuntimeExec(
  providerSessionId: string,
  ...
  const doRequest = async () => {
    ...
    const response = await (async () => {
      try {
        return await client.rpc.exec.$post(
          {
            json: {
              cmd: input.cmd,
              cwd: input.cwd,
  ...
  try {
    ...
    if (!(error instanceof SessionError)) {
      await ensureSessionRuntimeApiReady(providerSessionId, {
        deployIfNeeded: true,
        forceRestart: true,
      });
      return doRequest();
...
async function callSessionRuntimeAgent(
  providerSessionId: string,
  ...
  const doRequest = async () => {
    ...
    const response = await (async () => {
      try {
        return await client.rpc.agent.$post(
          {
            json: {
              prompt: input.prompt,
              cwd: input.cwd,
...
GIT_TERMINAL_PROMPT=0 git clone --depth 1 --branch "$REPO_REF" "$REPO_URL" "$WORKSPACE_PATH"
    ...
    if (cloneResult.exitCode !== 0) {
      ...
      await ensureSessionRuntimeApiReady(providerSession.sandboxId, {
        deployIfNeeded: true,
        forceRestart: true,
      });
    }
...
export async function executeInSession(
  ...
  try {
    return await callSessionRuntimeExec(record.providerSessionId, {
      cmd: command,
...
export async function runAgentInSession(
  ...
  try {
    return await callSessionRuntimeAgent(
      record.providerSessionId,
      {
        prompt,
        cwd: input.cwd?.trim() || record.workspacePath,
