...
- `apps/manager-app/src/server/slack/bot.ts`: Chat SDK bot instance (Slack adapter + Redis state) with event handlers for mentions, subscribed messages, and interactive button actions.
...
- `apps/manager-app/src/server/slack/notifier.ts`: Posts messages and approval cards to Slack threads from workflow steps using the Slack Web API via `fetch`. Extracts Slack-postable content directly from raw SDK messages.
...
- Slack layer: Chat SDK bot for incoming event routing; fetch-based Slack Web API for outgoing messages from workflow steps; Drizzle-based store for Slack ↔ Session/User mappings.
