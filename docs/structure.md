...
- `apps/session-runtime-api/dist`: Built artifacts for the session runtime API package (including bundled `server.js` used in Session bootstrap).
...
- `apps/manager-app/src/server/sessions/runtime-api.ts`: Session runtime API artifact deployment/startup, health checks, and private RPC client calls for `exec` / agent operations.
