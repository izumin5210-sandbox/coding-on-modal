...
- Slack integration uses Chat SDK (`chat` + `@chat-adapter/slack`) for incoming webhook handling and Redis (`@chat-adapter/state-redis`) for thread subscription state. Outgoing messages from the workflow use the Slack Web API directly via `fetch` to avoid circular dependencies and bot singleton management in workflow steps.
